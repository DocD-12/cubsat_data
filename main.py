...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def choose_txt(self):
        ...
        for i in range(1, lists[lists_num].max_row + 1):
            self.data_list_db_X.append(lists[lists_num][i][A_list].value)
            self.data_list_db_Y.append(lists[lists_num][i][B_list].value)
    ...
    def update_sun_activity(self):
        ...
        _filepath_sun_txt = QFileDialog.getOpenFileName(self, str("Загрузить txt файл солнечной активности"), "/", str("TEXT (*.xlsx)"))

        excel = openpyxl.open(_filepath_sun_txt[0], read_only=True)
        lists = excel.worksheets
        lists_num = 0
        A_list = 0
        B_list = 1
        for i in range(1, lists[lists_num].max_row + 1):
            self.data_list_sun_X.append(lists[lists_num][i][A_list].value)
            self.data_list_sun_Y.append(lists[lists_num][i][B_list].value)

        self.data_list_sun_X = list(map(float, self.data_list_sun_X))
        self.data_list_sun_Y = list(map(float, self.data_list_sun_Y))

        if (_filepath_sun_txt[0] != ""):
            ...
            # self.set_sun_graph(0, 0, True)
            self.compare_out(0, 0, True)
        self.filepath_sun_txt = _filepath_sun_txt
    ...
    def compare_click(self):
        if (self.filepath_txt[0] == ""):
            ...
            msg.exec_()
            self.compare_out(self.data_list_db_X, self.data_list_sun_Y, False) # Нужно передавать судя данные из алгоритма, которыми занимаются математики
            print(self.data_list_db_X, self.data_list_sun_Y)
        """
        else:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Critical)
    ...
    def compare_out(self, activity, power_db, clear): # Вывод графика зависимости солнечной активности от db
        if not(clear):
            self.ui.graph_out.plot(activity, power_db, pen=None, symbol='+')
        else:
